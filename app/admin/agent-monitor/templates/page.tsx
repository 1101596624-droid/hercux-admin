...
function ScoreBadge({ score }: { score: number }) {
  const s = score ?? 0;
  const color = s >= 85 ? 'bg-green-100 text-green-700'
    : s >= 75 ? 'bg-blue-100 text-blue-700'
    : s >= 60 ? 'bg-yellow-100 text-yellow-700'
    : 'bg-red-100 text-red-700';
  const grade = s >= 90 ? 'S' : s >= 85 ? 'A' : s >= 75 ? 'B' : s >= 60 ? 'C' : 'D';
  return (
...
function PreviewModal({ template, onClose }: { template: TemplateDetail; onClose: () => void }) {
  const scale = Math.min((window.innerWidth * 0.9) / 896, (window.innerHeight * 0.85) / 554);
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ transform: `scale(${scale})`, transformOrigin: 'center center' }}>
        <div className="bg-white rounded-t-2xl flex items-center justify-between px-6 py-3 border-b border-slate-200" style={{ width: 896 }}>
        ...
        <iframe
          ...
          style={{ width: 896, height: 504, border: 'none', display: 'block' }}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onCancel}>
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-[400px]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-end gap-3">
          ...
          <button onClick={onConfirm} className={`px-4 py-2 text-sm text-white rounded-lg transition-colors ${confirmColor}`}>
            {confirmText}
          </button>
        </div>
      </div>
...
export default function TemplatesPage() {
  ...
  const [confirmAction, setConfirmAction] = useState<{ type: 'approve' | 'reject' | 'delete'; template: Template } | null>(null);
  const [previewLoading, setPreviewLoading] = useState(false);

  const loadTemplates = useCallback(async () => {
    try {
      setLoading(true);
      const params = new URLSearchParams({ page: String(page), page_size: '12', status: statusTab });
      if (subjectFilter) params.set('subject', subjectFilter);
      const res = await fetch(`${API_BASE}/agent/templates?${params}`);
  ...
  }, [page, subjectFilter, statusTab]);

  const loadSubjects = async () => {
    try {
      const res = await fetch(`${API_BASE}/agent/templates/subjects/list`);
      const data = await res.json();
      setSubjects(data || []);
    } catch (err) {
  ...
  const loadPendingCount = async () => {
    try {
      const res = await fetch(`${API_BASE}/agent/templates/pending/count`);
      const data = await res.json();
  ...
  useEffect(() => { loadSubjects(); loadPendingCount(); }, []);
  useEffect(() => { loadTemplates(); }, [loadTemplates]);

  const handlePreview = async (id: number) => {
    try {
      setPreviewLoading(true);
      const res = await fetch(`${API_BASE}/agent/templates/${id}`);
      const data = await res.json();
  ...
  const handleApprove = async (id: number) => {
    try {
      await fetch(`${API_BASE}/agent/templates/${id}/approve`, { method: 'PUT' });
  ...
  const handleDelete = async (id: number) => {
    try {
      await fetch(`${API_BASE}/agent/templates/${id}`, { method: 'DELETE' });
      setConfirmAction(null);
      loadTemplates();
      loadPendingCount();
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-slate-500 mb-1">
            <Link href="/admin/agent-monitor" className="hover:text-slate-700">Agent 监控</Link>
            <span>/</span>
      ...
      {/* Status Tabs */}
      <div className="bg-white rounded-2xl p-1.5 shadow-sm border border-slate-100 inline-flex gap-1">
        <button
        ...
        >
          ...
          {pendingCount > 0 && (
            <span className={`ml-1.5 inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full text-xs font-bold ${
              statusTab === 'pending' ? 'bg-red-500 text-white' : 'bg-red-100 text-red-600'
            }`}>
              {pendingCount}
            </span>
          )}
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-xl transition-colors ${
            statusTab === 'approved' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
          已入库
        </button>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 shadow-sm border border-slate-100 text-center">
          <p className="text-slate-400 text-lg">
            {statusTab === 'pending' ? '暂无待审核模板' : '暂无已入库模板'}
          </p>
        </div>
      ) : (
                ...
                {statusTab === 'pending' ? (
                  <>
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 text-sm font-medium text-green-600 hover:bg-green-50 transition-colors text-center"
                    >
                      入库
                    </button>
                    <div className="w-px bg-slate-100"></div>
                    <button
                      onClick={() => setConfirmAction({ type: 'reject', template: t })}
      ...
      {confirmAction?.type === 'approve' && (
        <ConfirmModal
          title="确认入库"
          message={`确定要将模板「${confirmAction.template.topic}」入库吗？入库后 Agent 将可以学习此模板。`}
          confirmText="入库"
          confirmColor="bg-green-600 hover:bg-green-700"
          onConfirm={() => handleApprove(confirmAction.template.id)}
      ...
      {confirmAction?.type === 'reject' && (
        <ConfirmModal
          title="拒绝入库"
          message={`确定要拒绝模板「${confirmAction.template.topic}」吗？拒绝后模板将从列表中删除，课程中的代码不受影响。`}
          confirmText="拒绝"
          confirmColor="bg-red-600 hover:bg-red-700"
          onConfirm={() => handleDelete(confirmAction.template.id)}
          onCancel={() => setConfirmAction(null)}
        />
      )}
      {confirmAction?.type === 'delete' && (
        <ConfirmModal
          title="确认删除"
          message={`确定要删除模板「${confirmAction.template.topic}」吗？此操作不可撤销。`}
          confirmText="删除"
          confirmColor="bg-red-600 hover:bg-red-700"
          onConfirm={() => handleDelete(confirmAction.template.id)}
          onCancel={() => setConfirmAction(null)}
